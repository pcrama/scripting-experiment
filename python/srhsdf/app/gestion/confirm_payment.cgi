...
if __name__ == '__main__':
    ...
    try:
        ...
        if os.getenv('REQUEST_METHOD') == 'POST':
            ...
            if not token:
                redirect_to_event('1')
            payment = Payment.find_by_bank_ref(connection, form.getfirst('bank_ref', default=''))
            if not payment:
                redirect_to_event('line1')
            handle_post(
                connection,
                payment,
                token,
            ...
            if not reservation:
                redirect_to_event('line2')
            payment = Payment.find_by_bank_ref(connection, form.getfirst('bank_ref', default=''))
            if not payment:
                redirect_to_event('line3')
