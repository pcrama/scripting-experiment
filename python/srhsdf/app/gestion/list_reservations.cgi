...
def make_url(sort_order, limit, offset, base_url=None, environ=None):
    if base_url is None:
        ...
        protocol = 'https' if environ.get("SERVER_PORT") == 443 else 'http'
        base_url = f'{protocol}://{environ["SERVER_NAME"]}{environ["SCRIPT_NAME"]}'
    params = '&'.join(f'{k}={v}'
                      for k, v in
                      itertools.chain(
                          (('limit', limit),
...
if __name__ == '__main__':
    ...
    params = cgi.parse()
    sort_order = params.get('sort_order', '').split(',')
    try:
