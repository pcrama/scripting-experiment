...
class ImportBankStatements(unittest.TestCase):
    ...
    def test_non_overlapping_uploads(self):
        ...
        third_import = lib_payments.import_bank_statements(
            ...
            "third-user",
            "9.8.7.6")
        self.assertEqual(len(third_import), 4)
        self.assertIsNone(third_import[0][0])
        self.assertIsNotNone(third_import[1][0])
        self.assertIsNone(third_import[2][0])
        self.assertIsNotNone(third_import[3][0])
        self.assertEqual(
            storage.Payment.find_by_bank_ref(connection, third_import[-1][-1].bank_ref).src_id, f"{YEAR_PREFIX}-00999"
        )
        self.assertEqual(storage.Payment.length(connection), len(self.bank_statements_csv) - 1)
