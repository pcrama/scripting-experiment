...
class ExtractBankRef(unittest.TestCase):
    def test_example(self):
        self.assertEqual(lib_payments.extract_bank_ref('VIREMENT EN EUROS DU COMPTE BE00020002000202 BIC GABBBEBB CCCCC-CCCCCCCCC AV DE LA GARE 76 9999 WAGADOUGOU COMMUNICATION : REPRISE MARCHANDISES (VIANDE HACHEE) SOUPER ITALIEN REFERENCE BANQUE : 23032412000 DATE VALEUR : 28/03/2023'), '23032412000')

    def test_raises_when_no_left_marker(self):
        self.assertRaises(ValueError, lib_payments.extract_bank_ref, 'Tralala')

    def test_raises_when_no_right_marker(self):
        self.assertRaises(ValueError, lib_payments.extract_bank_ref, 'DATE VALEUR : 28/03/2023 SOUPER ITALIEN REFERENCE BANQUE : 23032412000 Trululu')
...
class ImportBankStatements(unittest.TestCase):
    ...
    def test_non_overlapping_uploads(self):
        ...
        first_import = lib_payments.import_bank_statements(connection, "\n".join(first_batch), "user-test", "1.2.3.4")
        self.assertEqual(len(first_import), len(first_batch) - 1)
        self.assertTrue(all(exc is None for exc, _ in first_import))
        self.assertEqual(storage.Payment.length(connection), len(first_batch) - 1)

        second_import = lib_payments.import_bank_statements(connection, "\n".join(second_batch), "other-user-test", "5.6.7.8")
        ...
        third_import = lib_payments.import_bank_statements(
            connection,
            "\n".join([self.bank_statements_csv[0]] + [
        ...
        self.assertEqual(
            storage.Payment.find_by_bank_ref(connection, third_import[-2][-1].bank_ref).src_id, f"{YEAR_PREFIX}-00999"
        )
        self.assertEqual(
            storage.Payment.find_by_bank_ref(connection, third_import[-1][-1].bank_ref).src_id, f"{YEAR_PREFIX}-00998"
        )
        self.assertEqual(storage.Payment.length(connection), len(self.bank_statements_csv) - 1)
    ...
    def test_payment_possible_bankid_but_no_reservation_match(self):
        ...
        for uuid in ('+++671/4235/58049+++', '671423558049'):
            with self.subTest(uuid=uuid):
                ...
                bank_ref += "2"
                with connection:
                    payment = conftest.make_payment(src_id=src_id, comment=uuid, uuid=None, bank_ref=bank_ref).insert_data(connection)
    ...
    def test_payment_possible_bankid_but_no_reservations_yet(self):
        ...
        for uuid in ('+++671/4235/58049+++', '671423558049'):
            with self.subTest(uuid=uuid):
                ...
                self.assertEqual(html_row, (('td', src_id if src_id else ('raw', '&nbsp;')),
                                            ('td', '17/01/2024'),
                                            ('td', 'BE0101'),
                                            ('td', 'Ms Abc'),
                                            ('td', 'Accepté'),
                ...
                src_id = "0" if src_id is None else (src_id + "0")
    ...
    def test_payment_already_linked_to_reservation(self):
        ...
        for email, expected_text in (("", 'testing'), (" ", "testing"), (None, "testing"), ("a@b.c", "testing a@b.c")):
            ...
            with self.subTest(email=email):
                with connection:
                    reservation = conftest.make_reservation(email=email, places=1, bank_id='bb' + uuid_hex, uuid=uuid_hex).insert_data(connection)
                    payment = conftest.make_payment(
                        uuid=reservation.uuid, src_id=src_id, confirmation_timestamp=1234.5, bank_ref=src_id
                    ).insert_data(connection)
                html_row = lib_payments.get_list_payments_row(connection, payment, reservation, 'example.com', 'srhsdf2024/gestion/list_payments.cgi', 'csrf_token_value')
                ...
                self.assertEqual(html_row, (('td', src_id),
                                            ...
                                            ('td',
                                             ('div',
                                              (('form', 'style', 'display: inline', 'method', 'POST', 'action', 'srhsdf2024/gestion/link_payment_and_reservation.cgi'),
                                               (('a', 'href', f'https://example.com/srhsdf2024/show_reservation.cgi?bank_id=bb{uuid_hex}&uuid_hex={uuid_hex}'),
                                                expected_text),
                                               ' ',
                                               (('input', 'type', 'hidden', 'name', 'csrf_token', 'value', 'csrf_token_value'),),
