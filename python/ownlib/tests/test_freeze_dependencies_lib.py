...
class TestsWithRealRepositories(unittest.TestCase):
    ...
    def test_given_clean_repository_when_calling_freeze_then_frozen(self):
        ...
        dependencies_file = self.parse(dependencies_lines)
        # When
    ...
    def test_given_dependency_not_cloned_when_calling_freeze_then_raises_exception(self):
        # Given
        common_root = path.dirname(self.repositories[1].working_dir)
        not_cloned = 'directory_does_not_exist'
        not_cloned_full_path = path.join(common_root, not_cloned)
        dependencies_lines = [
            not_cloned + ' ' + 'tag',
            path.basename(self.repositories[2].working_dir) + ' ' + self.tags[2]]
        dependencies_file = self.parse(dependencies_lines)
        ...
        with self.assertRaises(RuntimeError) as cm, \
            ...
            output = list(freeze_dependencies_list(
                main_project.working_dir, dependencies_file, False))
        # Then
        for fragment in (not_cloned_full_path, ' clone '):
            self.assertIn(fragment, cm.exception.args[0])
        for fragment in (common_root, self.repositories[2].working_dir):
            self.assertNotIn(fragment, sys_stdout.getvalue())
